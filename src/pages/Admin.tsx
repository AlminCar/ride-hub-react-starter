...
import { Shield, Users, MessageSquare, Settings, AlertTriangle, CheckCircle, XCircle } from "lucide-react";
...
const Admin = () => {
  ...
  const pendingUsers = [
    { id: 1, name: "John Doe", email: "john@example.com", phone: "+1234567890", status: "pending", joinDate: "2024-01-15" },
    { id: 2, name: "Jane Smith", email: "jane@example.com", phone: "+1234567891", status: "pending", joinDate: "2024-01-16" }
  ];

  const reportedIssues = [
    { id: 1, type: "ride", reporter: "Alice Johnson", subject: "Driver was late", status: "open", date: "2024-01-14" },
    { id: 2, type: "message", reporter: "Bob Wilson", subject: "Inappropriate content", status: "investigating", date: "2024-01-13" }
  ];
  ...
  const handleIssueResolution = (issueId: number, action: string) => {
    console.log(`Issue ${issueId} marked as ${action}`);
  };
                    ...
                    <div key={user.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-400">Applied: {new Date(user.joinDate).toLocaleDateString()}</p>
                        </div>
                      </div>
                    ...
                    <div key={issue.id} className="p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-2">
                          ...
                          <Badge className={issue.status === 'open' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}>
                            {issue.status}
                          </Badge>
                        </div>
                      ...
                      <div className="flex space-x-2 mt-3">
                        <Button size="sm" variant="outline" onClick={() => handleIssueResolution(issue.id, 'investigating')}>
                          Investigate
                        </Button>
                        <Button size="sm" variant="outline" onClick={() => handleIssueResolution(issue.id, 'resolved')}>
                          Mark Resolved
                        </Button>
                        <Button size="sm" variant="destructive" onClick={() => handleIssueResolution(issue.id, 'escalated')}>
                          Escalate
                        </Button>
                      </div>
                    </div>
          ...
          <TabsContent value="content" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Moderate messages and ride descriptions</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
          ...
          <TabsContent value="settings" className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="ride-notification">Ride Notification Template</Label>
                  <Textarea
